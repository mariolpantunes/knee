...
def multi_knee(get_knee: typing.Callable, points: np.ndarray, t1: float = 0.9, t2: int = 3) -> np.ndarray:
    """
    ...
    Args:
        ...
        t1 (float): the coefficient of determination used as a threshold (default 0.9)
...
def multi_knee_rec(get_knee: typing.Callable, points: np.ndarray, left: int, right: int,  t1: float = 0.9, t2: int = 3) -> np.ndarray:
    """
    Wrapper that convert a single knee point detection into a multi knee point detector.
        ...
        if linear_r2_points(pt, coef) < t1:
            ...
            if rv is not None:
                ...
                left_knees = multi_knee_rec(
                    get_knee, points, left, idx+1, t1, t2)
                right_knees = multi_knee_rec(
                    get_knee, points, idx+1, right, t1, t2)
